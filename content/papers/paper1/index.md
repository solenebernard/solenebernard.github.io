...
$\mathcal{Z}^{0}$ initial stego base, $\mathcal{X} =\left\lbrace \mathbf{x}_{(1)},..,\mathbf{x}_{(N)} \right\rbrace$ cover base, set of detectors $\mathcal{F}^0 = \left\lbrace f^0 \right\rbrace, k_{\max}$
